    ...
    public ResourceNotification persist(ResourceNotification notification) {
        ...
        List<Resource> resources = cloudResources.stream()
                .filter(cr -> {
                    ...
                    if (exist) {
                        LOGGER.debug("{} and {} is already exists in DB for stack.", cr.getReference(), cr.getType());
                    }
                    return !exist;
                })
                .map(cloudResource -> {
        ...
        if (cloudResources.size() != resources.size()) {
            LOGGER.debug("There are {} resource(s), these will not save", cloudResources.size() - resources.size());
        }
