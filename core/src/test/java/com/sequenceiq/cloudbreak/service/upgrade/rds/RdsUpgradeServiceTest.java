...
class RdsUpgradeServiceTest {
    ...
    void testUpgradeRdsWithValidSetupThenSuccess() {
        ...
        when(databaseService.getDatabaseServer(eq((STACK_CRN)))).thenReturn(createDatabaseServerResponse(MajorVersion.VERSION_10));
        ...
        when(reactorNotifier.notify(eq(STACK_ID), eq("UPGRADE_RDS_TRIGGER_EVENT"), any(Acceptable.class))).thenReturn(flowId);

        RdsUpgradeV4Response response = underTest.upgradeRds(NameOrCrn.ofCrn(STACK_CRN), TARGET_VERSION);

        ArgumentCaptor<UpgradeRdsTriggerRequest> requestCaptor = ArgumentCaptor.forClass(UpgradeRdsTriggerRequest.class);
        verify(reactorNotifier).notify(eq(STACK_ID), eq("UPGRADE_RDS_TRIGGER_EVENT"), requestCaptor.capture());
        UpgradeRdsTriggerRequest request = requestCaptor.getValue();
    ...
    void testWhenRdsAlreadyUpgradedThenError() {
        Stack stack = createStack(Status.AVAILABLE);
        when(databaseService.getDatabaseServer(eq((STACK_CRN)))).thenReturn(createDatabaseServerResponse(MajorVersion.VERSION_11));
        when(stackService.getByNameOrCrnInWorkspace(eq(NameOrCrn.ofCrn(STACK_CRN)), any())).thenReturn(stack);
        when(messagesService.getMessage(CLUSTER_RDS_UPGRADE_ALREADY_UPGRADED.getMessage(), List.of(MajorVersion.VERSION_11.getVersion()))).thenReturn(ERROR_REASON);
