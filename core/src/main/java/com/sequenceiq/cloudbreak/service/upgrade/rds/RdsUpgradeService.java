...
public class RdsUpgradeService {
    ...
    public RdsUpgradeV4Response upgradeRds(NameOrCrn nameOrCrn, TargetMajorVersion targetMajorVersion) {
        ...
        if (checkCurrentRdsVersion(nameOrCrn.getCrn()).equals(targetMajorVersion.getVersion())) {
    ...
    private String checkCurrentRdsVersion(String crn) {
        StackDatabaseServerResponse databaseServer = databaseService.getDatabaseServer(crn);
    ...
    private RdsUpgradeV4Response triggerRdsUpgradeFlow(Stack stack, TargetMajorVersion targetMajorVersion) {
        String selector = UPGRADE_RDS_EVENT.event();
        FlowIdentifier triggeredFlowId = reactorNotifier.notify(stack.getId(), selector, new UpgradeRdsTriggerRequest(stack.getId(), targetMajorVersion));
